# FOOD KCN Android v1.8.2 — Production Release

## Ghi chú bản 1.8.2 (so với 1.7.0)
...
## Lịch sử bản 1.7.0 (so với 1.6.0)
...
## Lịch sử bản 1.6.0 (so với 1.5.2)
...
## Lịch sử bản 1.4.2 (so với 1.4.1)
...
## Lịch sử bản 1.4.1 (sửa lỗi so với 1.4.0)
...
published as the workflow artifact `FOOD-KCN-v1.8.2-release-AAB`
...
## Bản 1.8.2 FINAL — lưu ý phát hành
- `versionCode = 15`, `versionName = 1.8.2`.
- `compileSdk = 36`, `targetSdk = 36`, `minSdk = 23`. Google Play hiện yêu cầu app mới/cập nhật gửi lên Play phải target API 36 trở lên từ 31/08/2026.
- Đã loại bỏ toàn bộ module video/Tin Tức/livestream và cơ chế bong bóng video khỏi bản Android này.
- Release workflow tạo AAB ký bằng đúng release keystore từ GitHub Secrets.
- Workflow dùng `storeFile=com11h-release.jks` đúng vị trí file keystore được tạo trong thư mục `android`.
- Không lưu mật khẩu đăng nhập trên thiết bị; chỉ ghi nhớ số điện thoại.
- Luồng xóa tài khoản có cả đường dẫn trong app và web; backend phải xử lý `delete_account` thật sự trước khi phát hành.
