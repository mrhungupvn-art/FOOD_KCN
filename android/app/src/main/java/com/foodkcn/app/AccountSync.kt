...
class AccountSync(context: Context) {
    ...
    /**
     * Lưu toạ độ GPS hiện tại của khách lên server (để order_preview và các
     * lần gọi 'menu' sau vẫn tính được km/phí ship kể cả khi app không kèm
     * lat/lng trực tiếp trong request đó). Chỉ gọi được khi đã đăng nhập.
     */
    fun updateLocation(lat: Double, lng: Double): JSONObject = request(
        action = "update_location",
        method = "POST",
        body = JSONObject().put("lat", lat).put("lng", lng).toString()
    )
