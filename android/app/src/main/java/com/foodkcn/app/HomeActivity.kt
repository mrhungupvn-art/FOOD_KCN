...
class HomeActivity : SessionActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState); account = AccountSync(this); xuApi = XuApi(account); selectedKcnId = KcnStore.id(this); selectedKcnName = KcnStore.name(this)
        showSplash()
    }
     ...
     * xem đủ 10 món khác nhau được thưởng thêm 100 Xu — theo README XU). Tự
     * ẩn nếu khách đã đạt giới hạn Xu trong giờ/ngày, để không nhắc nhở vô
     * ích khi không thể nhận thêm Xu nữa.
     *
     * Số Xu/giới hạn lấy THẬT từ server (api action 'xu_wallet') qua XuApi,
     * giống hệt màn "Ví XU" ở MainActivity — không còn đọc bộ nhớ tạm trên
     * máy (XuStore) nữa, để bong bóng này luôn khớp với số Xu thật của khách
     * thay vì một số đếm cục bộ có thể lệch hoặc bị can thiệp trên máy.
     */
    private fun xuReminderBubble(): View {
        ...
        // Khách chưa đăng nhập thì chưa có ví Xu trên server -> vẫn hiện bong
        // bóng để mời khách bấm vào (mở Thực đơn), màn Chi tiết món sẽ tự mời
        // đăng nhập khi khách bắt đầu xem. Chỉ khách ĐÃ đăng nhập mới cần hỏi
        // server xem đã đạt giới hạn giờ/ngày để quyết định ẩn hay không.
        if (account.isLoggedIn()) {
            executor.execute {
                val walletResult = try { xuApi.wallet() } catch (_: Exception) { null }
                runOnUiThread {
                    if (isFinishing || isDestroyed) return@runOnUiThread
                    val ok = walletResult?.optBoolean("ok") == true
                    if (!ok) return@runOnUiThread // Lỗi mạng/server -> để yên bong bóng, đừng đoán mò.
                    ...
                    val dayXu = today.optInt("earned", 0); val dayLimit = today.optInt("limit", 2000)
                    val hourXu = thisHour.optInt("earned", 0); val hourLimit = thisHour.optInt("limit", 200)
                    if (dayXu >= dayLimit || hourXu >= hourLimit) bubble.visibility = View.GONE
                }
            }
        }
        return bubble
