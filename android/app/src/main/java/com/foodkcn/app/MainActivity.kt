...
class MainActivity : SessionActivity() {
    ...
    companion object {
        private const val REQUEST_LOCATION_MENU = 4301
    }

    // Khách vừa cấp quyền định vị ở màn Thực đơn -> tải lại ngay để hiện km/phí
    // ship trên từng món (không cần khách tự kéo làm mới hoặc thoát vào lại).
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        super.onRequestPermissionsResult(requestCode, permissions, grantResults)
        if (requestCode == REQUEST_LOCATION_MENU && LocationHelper.hasPermission(this)) {
            showMenu()
        }
    }
    ...
    data class Food(
        ...
        // km / phí ship của TIỆM bán món này tới khách — server chỉ tính được
        // khi biết toạ độ khách (xem showMenu: gửi kèm lat/lng thiết bị). Null
        // nghĩa là chưa xác định được (chưa cấp quyền định vị / chưa bật GPS).
        val shipping: Shipping? = null
    )

    data class Shipping(
        val distanceKm: Double, val fee: Int, val normalFee: Int, val discount: Int,
        val freeShipping: Boolean, val storeName: String?
    )
    ...
    private fun showMenu() {
        ...
        // Chưa có quyền định vị thì xin ngay ở đây (màn Thực đơn là nơi km/phí
        // ship cần hiển thị trên từng món) — không chặn tải thực đơn nếu khách
        // từ chối, chỉ là lúc đó sẽ không có cột km/phí ship trên mỗi món.
        if (!LocationHelper.hasPermission(this)) {
            requestPermissions(
                arrayOf(android.Manifest.permission.ACCESS_FINE_LOCATION, android.Manifest.permission.ACCESS_COARSE_LOCATION),
                REQUEST_LOCATION_MENU
            )
        }

        executor.execute {
            try {
                val loc = LocationHelper.currentLocationBlocking(this)
                val query = mutableMapOf("kcn_id" to KcnStore.id(this).toString())
                if (loc != null) {
                    query["lat"] = loc.latitude.toString()
                    query["lng"] = loc.longitude.toString()
                    // Lưu luôn lên tài khoản (nếu đã đăng nhập) để lần đặt hàng
                    // (order_preview) sau cũng tính được phí ship, không chỉ màn
                    // Thực đơn này. Không cần chờ / không quan trọng nếu lỗi.
                    if (account.isLoggedIn()) {
                        try { account.updateLocation(loc.latitude, loc.longitude) } catch (_: Exception) {}
                    }
                }
                val r = account.request("menu", query = query)
                runOnUiThread {
                    ...
                    for (i in 0 until arr.length()) {
                        ...
                        val shippingObj = f.optJSONObject("shipping")
                        val shipping = shippingObj?.let {
                            Shipping(
                                distanceKm = it.optDouble("distance_km", 0.0),
                                fee = it.optInt("fee", 0),
                                normalFee = it.optInt("normal_fee", it.optInt("fee", 0)),
                                discount = it.optInt("discount", 0),
                                freeShipping = it.optBoolean("free_shipping", false),
                                storeName = f.optString("store_name").takeIf { n -> n.isNotBlank() }
                            )
                        }
                        foods.add(Food(f.optInt("id"), f.optString("name"), f.optInt("price"), f.optInt("stock"), f.optString("category"), f.optString("description"), f.optString("image"), shipping))
                    }
                    foodsCache = foods
    ...
    /** Dòng "📍 2.3km • 🚚 Phí ship: 15.000đ" (hoặc "Miễn phí ship") hiện dưới giá mỗi món. */
    private fun shippingLabel(sh: Shipping): TextView {
        val distanceText = String.format("%.1f", sh.distanceKm).replace('.', ',') + "km"
        val feeText = when {
            sh.freeShipping -> "Miễn phí ship"
            sh.discount > 0 -> "Phí ship: ${money(sh.fee)} (giảm ${money(sh.discount)})"
            else -> "Phí ship: ${money(sh.fee)}"
        }
        return label("📍 $distanceText  •  🚚 $feeText", 12.5f, if (sh.freeShipping || sh.discount > 0) primary else secondary, sh.freeShipping || sh.discount > 0)
    }
    ...
    private fun foodCard(f: Food, listContext: List<Food> = listOf(f), indexInList: Int = 0, backTo: () -> Unit = { showMenu() }): LinearLayout {
        ...
        // km + phí ship của tiệm bán món này tới khách — chỉ có khi server xác
        // định được toạ độ khách (xem showMenu: gửi kèm lat/lng thiết bị). Nếu
        // f.shipping == null (chưa cấp quyền định vị / chưa bắt được GPS) thì
        // không hiện dòng này, tránh làm rối giao diện với thông tin thiếu.
        f.shipping?.let { sh -> info.addView(shippingLabel(sh)) }
    ...
    private fun showFoodDetail(f: Food) {
        ...
        f.shipping?.let { sh -> c.addView(shippingLabel(sh).apply { textSize = 14f }) }
    ...
    private fun reverseGeocodeForCheckout(lat: Double, lng: Double): String {
        return try {
            ...
            a.getAddressLine(0)?.trim()?.takeIf { it.isNotBlank() }?.let { parts.add(it) }
            if (parts.isEmpty()) {
                listOf(a.subThoroughfare, a.thoroughfare, a.subLocality, a.locality, a.adminArea)
                    .mapNotNull { it?.trim()?.takeIf { x -> x.isNotBlank() } }
    ...
    private fun showCheckout() {
        ...
        var deliveryAccuracy: Double? = null
        ...
        noticeBox.addView(label(
            "Trước khi tính phí ship, hãy chọn đúng vị trí bạn đang đứng. Ứng dụng sẽ lấy GPS của điện thoại, hiển thị địa chỉ để bạn kiểm tra; phí ship sẽ được tính riêng cho từng cửa hàng trong giỏ.",
        ...
        val address = EditText(this).apply {
            hint = "Địa chỉ giao hàng (sẽ tự lấy từ GPS)"
            setText(lastAddress()); textSize = 15f
            setPadding(dp(12), dp(10), dp(12), dp(10)); background = bg(Color.WHITE, 12)
        }
        val locationStatus = label("📍 Chưa chọn vị trí GPS", 13f, danger, true)
        lateinit var locationBtn: TextView
        locationBtn = button("📍 Chọn vị trí bạn đang đứng") {
            if (!LocationHelper.hasPermission(this@MainActivity)) {
                requestPermissions(arrayOf(android.Manifest.permission.ACCESS_FINE_LOCATION, android.Manifest.permission.ACCESS_COARSE_LOCATION), REQUEST_LOCATION_MENU)
                toast("Vui lòng cấp quyền vị trí rồi bấm lại nút.")
                return@button
            }
            locationStatus.text = "⏳ Đang xác định vị trí…"
            locationBtn.isEnabled = false
            executor.execute {
                val loc = LocationHelper.currentLocationBlocking(this@MainActivity, 10000L)
                if (loc == null) {
                    runOnUiThread {
                        locationStatus.text = "❌ Không lấy được GPS. Hãy bật định vị và thử lại."
                        locationBtn.isEnabled = true
                    }
                    return@execute
                }
                val addr = reverseGeocodeForCheckout(loc.latitude, loc.longitude)
                runOnUiThread {
                    deliveryLat = loc.latitude
                    deliveryLng = loc.longitude
                    deliveryAccuracy = loc.accuracy.toDouble()
                    if (addr.isNotBlank()) address.setText(addr)
                    locationStatus.text = if (addr.isNotBlank())
                        "✓ Vị trí đã chọn: $addr (độ chính xác ~${loc.accuracy.toInt()}m)"
                    else
                        "✓ Đã chọn GPS (${String.format("%.6f", loc.latitude)}, ${String.format("%.6f", loc.longitude)}). Bạn có thể chỉnh địa chỉ."
                    locationStatus.setTextColor(ok)
                    locationBtn.isEnabled = true
                }
            }
        }
        c.addView(locationBtn, LinearLayout.LayoutParams(-1,-2).apply { bottomMargin=dp(6) })
        c.addView(locationStatus, LinearLayout.LayoutParams(-1,-2).apply { bottomMargin=dp(8) })
        val time = EditText(this).apply {
            hint = "Giờ giao hàng (ví dụ : 11h30, bắt buộc)"; textSize = 15f
            setPadding(dp(12), dp(10), dp(12), dp(10)); background = bg(Color.WHITE, 12)
        }
        val recipientPhone = EditText(this).apply {
            hint = "Số điện thoại người nhận (bắt buộc)"; textSize = 15f
            inputType = android.text.InputType.TYPE_CLASS_PHONE
            setPadding(dp(12), dp(10), dp(12), dp(10)); background = bg(Color.WHITE, 12)
        }
        val note = EditText(this).apply {
            hint = "Ghi chú (không bắt buộc)"; textSize = 15f
            setPadding(dp(12), dp(10), dp(12), dp(10)); background = bg(Color.WHITE, 12)
        }
        listOf(address, time, recipientPhone, note).forEach { c.addView(it, LinearLayout.LayoutParams(-1, -2).apply { bottomMargin = dp(10) }) }
        ...
        val previewBtn = button("Xem lại tổng tiền") {
            if (deliveryLat == null || deliveryLng == null) { toast("Vui lòng chọn vị trí bạn đang đứng để tính phí ship"); return@button }
            if (address.text.toString().trim().isBlank()) { toast("Chưa lấy được địa chỉ. Vui lòng nhập địa chỉ giao hàng"); return@button }
            if (recipientPhone.text.toString().trim().isBlank()) { toast("Vui lòng nhập số điện thoại người nhận"); return@button }
            ...
            executor.execute {
                val body = JSONObject()
                    ...
                    .put("phone", recipientPhone.text.toString().trim())
                    .put("delivery_time", time.text.toString().trim())
                    .put("lat", deliveryLat)
                    .put("lng", deliveryLng)
                    .put("accuracy_m", deliveryAccuracy)
                ...
                runOnUiThread {
                    ...
                    val breakdown = shipping.optJSONArray("breakdown")
                    if (breakdown != null && breakdown.length() > 0) {
                        summaryBox.addView(label("🚚 Phí vận chuyển theo từng cửa hàng", 15f, dark, true))
                        for (bi in 0 until breakdown.length()) {
                            val q = breakdown.optJSONObject(bi) ?: continue
                            val storeName = q.optString("store_name", "Cửa hàng")
                            val km = q.optDouble("distance_km", Double.NaN)
                            val fee = q.optInt("shipping_fee", 0)
                            val free = q.optBoolean("free_shipping", false)
                            val kmText = if (km.isNaN()) "" else " • ${String.format("%.1f", km)} km"
                            summaryBox.addView(label(
                                "• $storeName$kmText: " + if (free) "FREE SHIP" else money(fee),
                                14f, if (free) ok else secondary, free
                            ))
                        }
                    }
