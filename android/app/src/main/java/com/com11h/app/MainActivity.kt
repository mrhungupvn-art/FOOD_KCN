...
class MainActivity : SessionActivity() {
    ...
    // =========================================================================
    // CHI TIẾT MÓN + TÍCH XU: đủ 30 giây xem một món -> 10 XU (bản test local).
    ...
    private fun showFoodDetail(f: Food) {
        ...
        val already = XuStore.watched(this, f.id)
        if (already) timer.text = "✓ Bạn đã nhận XU từ món này"
        else {
            ...
            val runnable = object : Runnable {
                override fun run() {
                    ...
                    if (elapsed >= 30) {
                        val earned = XuStore.reward(this@MainActivity, f.id)
                        timer.text = if (earned > 0) "🎉 +$earned XU!" else "⚠️ Đã đạt giới hạn XU hôm nay/giờ này"
                        return
                    }
                    timer.text = "⏱ Còn ${30-elapsed} giây để nhận 10 XU"
    ...
    private fun showXu() {
        ...
        val s=shell("Ví XU",4); setContentView(s); val c=contentOf(s); val st=XuStore.state(this)
        ...
        card.addView(label("${String.format("%,d",st.balance)} XU",30f,primary,true))
        card.addView(label("Hôm nay: ${st.dayXu}/2.000 XU",14f,secondary))
        card.addView(label("Giờ này: ${st.hourXu}/200 XU",14f,secondary))
        ...
        val vouchers=listOf(500 to "Voucher 5.000đ",1000 to "Voucher 10.000đ",2000 to "Voucher 25.000đ")
        vouchers.forEach{(cost,name)->c.addView(ghostButton("$cost XU → $name"){ if(XuStore.redeem(this,cost)){toast("Đã đổi $name");showXu()}else toast("Bạn chưa đủ XU") }.apply{layoutParams=LinearLayout.LayoutParams(-1,-2).apply{bottomMargin=dp(7)}})}
        c.addView(label("ℹ️ Đây là cơ chế XU bản test trên thiết bị. Khi API XU trên server được tích hợp, số dư sẽ chuyển sang lưu và kiểm tra hoàn toàn trên máy chủ để chống gian lận.",12.5f,secondary).apply{setPadding(0,dp(14),0,0)})
    }
            ...
            try {
                ...
                runOnUiThread {
                    ...
                    if (r.optBoolean("ok")) {
                        ...
                        c.addView(label("🪙 Ví XU: ${XuStore.state(this@MainActivity).balance} XU", 18f, primary, true))
...
class MainActivity : SessionActivity() {
    ...
    private fun foodCard(f: Food): LinearLayout {
        ...
        card.addView(info, LinearLayout.LayoutParams(0, -2, 1f))
        info.setOnClickListener { showFoodDetail(f) }
        card.setOnClickListener { showFoodDetail(f) }
    ...
    private fun showFoodDetail(f: Food) {
        ...
        c.addView(img, LinearLayout.LayoutParams(-1, dp(230)).apply { bottomMargin = dp(12) }); ImageLoader.load(img, f.image)
        img.setOnClickListener { openFoodImage(f.image, f.name) }
        c.addView(label(f.name, 23f, dark, true)); c.addView(label(money(f.price), 20f, primary, true)); c.addView(label("Còn ${f.stock} phần", 13f, secondary))
        if (f.description.isNotBlank()) c.addView(label(f.description, 15f, dark))
        val info = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; background = bg(Color.rgb(255,250,236),16); setPadding(dp(14),dp(12),dp(14),dp(12)) }
        info.addView(label("🪙 Nhận XU khi xem món", 17f, dark, true))
        val timer = label("⏳ Đang tạo phiên xem...", 14f, primary, true); info.addView(timer)
        c.addView(info, LinearLayout.LayoutParams(-1,-2).apply { bottomMargin = dp(12) })
        if (!account.isLoggedIn()) {
            timer.text = "🔐 Đăng nhập để nhận XU"; c.addView(button("Đăng nhập / Đăng ký") { showLogin() })
        } else {
            executor.execute {
                try {
                    val startR = XuApi(account).startView(f.id); val data = startR.optJSONObject("data"); val viewId = data?.optInt("view_id",0) ?: 0; val required = data?.optInt("required_seconds",30) ?: 30
                    runOnUiThread {
                        if (!startR.optBoolean("ok") || viewId <= 0) { timer.text = startR.optString("message","Không thể bắt đầu phiên xem XU."); return@runOnUiThread }
                        timer.text = "⏱ Còn $required giây để nhận XU"
                        val started = System.currentTimeMillis()
                        val task = object: Runnable { override fun run() {
                            val elapsed=((System.currentTimeMillis()-started)/1000).toInt()
                            if(elapsed>=required){
                                timer.text="⏳ Đang xác nhận XU..."
                                executor.execute {
                                    try { val done=XuApi(account).completeView(viewId); runOnUiThread {
                                        if(done.optBoolean("ok")){ val d=done.optJSONObject("data"); val earned=d?.optInt("earned",0)?:0; val products=d?.optInt("products_today",0)?:0; timer.text=if(earned>0)"🎉 +$earned XU! Hôm nay đã xem $products món" else done.optString("message","Đã kiểm tra phiên xem.") }
                                        else timer.text=done.optString("message","Chưa thể nhận XU.")
                                    }} catch(_:Exception){runOnUiThread{timer.text="⚠️ Không thể kết nối máy chủ XU."}}
                                }; return
                            }
                            timer.text="⏱ Còn ${required-elapsed} giây để nhận XU"; handler.postDelayed(this,1000)
                        }}
                        handler.post(task)
                    }
                } catch(_:Exception){runOnUiThread{timer.text="⚠️ Không thể kết nối máy chủ XU."}}
        ...
        c.addView(button("🪙 Xem ví XU") { showXu() }.apply { layoutParams=LinearLayout.LayoutParams(-1,-2).apply{topMargin=dp(8)} })
        c.addView(button(if(f.stock>0)"🛒 Thêm vào giỏ" else "Hết hàng") {
            if(f.stock<=0){toast("Món này đã hết hàng");return@button}; val q=cart[f.id]?:0; if(q+1>f.stock){toast("Chỉ còn ${f.stock} phần");return@button}; cart[f.id]=q+1; saveLocalCart(); refreshCartBadge(); toast("Đã thêm ${f.name} vào giỏ hàng")
        }.apply{layoutParams=LinearLayout.LayoutParams(-1,-2).apply{topMargin=dp(8)}})
    }

    // =========================================================================
    // VÍ XU — số dư, lịch sử và đổi ưu đãi lấy từ SERVER
    // =========================================================================
    private fun showXu() {
        if(!account.isLoggedIn()){toast("Vui lòng đăng nhập để sử dụng XU");showLogin();return}
        val s=shell("Ví XU",4); setContentView(s); val c=contentOf(s); val loading=label("⏳ Đang đồng bộ ví XU...",15f,secondary); c.addView(loading)
        executor.execute {
            try {
                val api=XuApi(account); val r=api.wallet(); val d=r.optJSONObject("data"); val w=d?.optJSONObject("wallet"); val today=d?.optJSONObject("today"); val hour=d?.optJSONObject("this_hour"); val settings=d?.optJSONObject("settings"); val rewards=api.rewards().optJSONObject("data")?.optJSONArray("rewards") ?: org.json.JSONArray()
                runOnUiThread {
                    c.removeView(loading); if(!r.optBoolean("ok")||d==null){c.addView(label(r.optString("message","Không thể tải ví XU."),15f,danger));return@runOnUiThread}
                    val card=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;background=bg(Color.WHITE,18);setPadding(dp(16),dp(16),dp(16),dp(16))}; card.addView(label("${String.format("%,d",w?.optInt("balance",0)?:0)} XU",30f,primary,true)); card.addView(label("Hôm nay: ${today?.optInt("earned",0)?:0}/${today?.optInt("limit",2000)?:2000} XU",14f,secondary)); card.addView(label("Giờ này: ${hour?.optInt("earned",0)?:0}/${hour?.optInt("limit",200)?:200} XU",14f,secondary)); c.addView(card,LinearLayout.LayoutParams(-1,-2).apply{bottomMargin=dp(12)})
                    c.addView(label("🎯 Cách nhận XU",18f,dark,true)); c.addView(label("• Xem đủ ${settings?.optInt("view_seconds",30)?:30} giây → +${settings?.optInt("view_reward",10)?:10} XU\n• Đủ 10 sản phẩm khác nhau trong ngày → thưởng thêm ${settings?.optInt("ten_products_reward",100)?:100} XU\n• Tối đa ${hour?.optInt("limit",200)?:200} XU mỗi giờ\n• Tối đa ${today?.optInt("limit",2000)?:2000} XU mỗi ngày",14f,secondary))
                    c.addView(label("🎁 Đổi XU",18f,dark,true).apply{setPadding(0,dp(16),0,dp(6))})
                    for(i in 0 until rewards.length()){ val rw=rewards.optJSONObject(i)?:continue; val id=rw.optInt("id",0); val cost=rw.optInt("xu_cost",0); val title=rw.optString("title","Ưu đãi"); c.addView(ghostButton("$cost XU → $title"){executor.execute{try{val rr=api.redeem(id);runOnUiThread{Toast.makeText(this,rr.optString("message",if(rr.optBoolean("ok"))"Đổi thành công" else "Không thể đổi XU")+(if(rr.optBoolean("ok"))"\nMã: "+rr.optJSONObject("data")?.optString("claim_code","") else ""),Toast.LENGTH_LONG).show();if(rr.optBoolean("ok"))showXu()}}catch(_:Exception){runOnUiThread{toast("Không thể kết nối máy chủ")}}}}.apply{layoutParams=LinearLayout.LayoutParams(-1,-2).apply{bottomMargin=dp(7)}})}
                    if(rewards.length()==0)c.addView(label("Hiện chưa có phần thưởng đổi XU.",14f,secondary))
                    c.addView(ghostButton("📜 Lịch sử XU"){executor.execute{try{val h=api.history();val arr=h.optJSONObject("data")?.optJSONArray("transactions")?:org.json.JSONArray();val lines=StringBuilder("📜 LỊCH SỬ XU\n\n");for(i in 0 until minOf(arr.length(),50)){val x=arr.optJSONObject(i)?:continue;val sign=if(x.optString("type")=="spend")"-" else "+";lines.append(sign).append(x.optInt("amount")).append(" XU — ").append(x.optString("reason")).append("\n")};runOnUiThread{Toast.makeText(this,lines.toString(),Toast.LENGTH_LONG).show()}}catch(_:Exception){runOnUiThread{toast("Không thể tải lịch sử XU")}}}}.apply{layoutParams=LinearLayout.LayoutParams(-1,-2).apply{topMargin=dp(8)}})
                }
            }catch(_:Exception){runOnUiThread{c.removeView(loading);c.addView(label("⚠️ Không thể kết nối máy chủ XU.",15f,danger))}}
        }
    }
    ...
    private fun renderOrderDetail(box: LinearLayout, code: String) {
        executor.execute {
            ...
            runOnUiThread {
                ...
                box.addView(label("Tổng cộng: ${money(o.optInt("total"))}", 18f, primary, true).apply { setPadding(0, dp(10), 0, dp(4)) })
                box.addView(button("🔄 Mua lại đơn này") {
                    var added = 0
                    for (i in 0 until items.length()) {
                        val it = items.getJSONObject(i)
                        val id = it.optInt("food_id", it.optInt("product_id", it.optInt("id", 0)))
                        val qty = it.optInt("qty", 1)
                        if (id > 0 && qty > 0) { cart[id] = (cart[id] ?: 0) + qty; added += qty }
                    }
                    saveLocalCart(); refreshCartBadge()
                    toast(if (added > 0) "Đã thêm $added món vào giỏ hàng" else "Đơn này không có mã món để mua lại")
                    if (added > 0) showCart()
                }.apply { layoutParams = LinearLayout.LayoutParams(-1, -2).apply { topMargin = dp(8) } })
    ...
    private fun showFavorites() {
        ...
        executor.execute {
            ...
            runOnUiThread {
                ...
                else foods.forEach { box.addView(foodCard(it)) }
            }
        }
    }
            ...
            try {
                ...
                runOnUiThread {
                    ...
                    if (r.optBoolean("ok")) {
                        ...
                        c.addView(label("Họ tên: ${customer.optString("name", "Chưa cập nhật")}", 17f, dark, true))
                        c.addView(label("Số điện thoại: ${customer.optString("phone", "")}"))
                        c.addView(label("🪙 Ví XU: đang đồng bộ từ máy chủ...", 18f, primary, true))
                        c.addView(label("Toàn bộ đơn hàng, thanh toán và mã quay thưởng của bạn được đồng bộ trực tiếp với tài khoản trên website.", 12.5f, secondary))
