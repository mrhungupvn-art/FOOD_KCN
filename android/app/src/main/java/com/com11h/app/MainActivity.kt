...
class MainActivity : Activity() {
    ...
    private fun ghostButton(v: String, click: () -> Unit) = TextView(this).apply { text = v; textSize = 14f; gravity = Gravity.CENTER; setTextColor(primary); background = outline(primary, 13); setPadding(dp(12), dp(11), dp(12), dp(11)); setOnClickListener { click() } }
                    ...
                    fun renderChips() {
                        ...
                        categories.forEach { cat ->
                            chipsRow.addView(TextView(this@MainActivity).apply {
                                text = cat; textSize = 13f; gravity = Gravity.CENTER
    ...
    private fun foodCard(f: Food): LinearLayout {
        ...
        card.addView(img, LinearLayout.LayoutParams(dp(76), dp(76)))
        ImageLoader.load(img, f.image)
        val info = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(11), 0, dp(6), 0) }
        info.addView(label(f.name, 16f, dark, true))
        if (f.description.isNotBlank()) info.addView(label(f.description, 12.5f, secondary))
        info.addView(label("${money(f.price)}   •   còn ${f.stock} phần", 14f, primary, true))
        ...
        val addBtn = TextView(this).apply {
            ...
            setOnClickListener {
                ...
                cart[f.id] = current + 1; saveLocalCart(); toast("Đã thêm \"${f.name}\" vào giỏ hàng")
            }
        }
    ...
    private fun showCart() {
        ...
        fun renderCart(foods: List<Food>) {
            ...
            cart.toMap().forEach { (id, qty) ->
                ...
                stepper.addView(TextView(this).apply { text = "−"; textSize = 18f; setTextColor(primary); setTypeface(null, Typeface.BOLD); setPadding(dp(10), dp(4), dp(10), dp(4)); setOnClickListener { if (qty <= 1) cart.remove(id) else cart[id] = qty - 1; saveLocalCart(); renderCart(foods) } })
                stepper.addView(label("$qty", 15f, dark, true))
                stepper.addView(TextView(this).apply { text = "+"; textSize = 18f; setTextColor(primary); setTypeface(null, Typeface.BOLD); setPadding(dp(10), dp(4), dp(10), dp(4)); setOnClickListener { if (qty + 1 > f.stock) { toast("Chỉ còn ${f.stock} phần"); return@setOnClickListener }; cart[id] = qty + 1; saveLocalCart(); renderCart(foods) } })
    ...
    // Đặt hàng: nhập địa chỉ -> order_preview (server tính lại giá/tồn kho) -> create_order (kèm Idempotency-Key).
    private fun showCheckout() {
        ...
        val address = EditText(this).apply { hint = "Địa chỉ giao hàng"; setText(lastAddress()); textSize = 15f; setPadding(dp(12), dp(10), dp(12), dp(10)); background = bg(Color.WHITE, 12) }
        val time = EditText(this).apply { hint = "Giờ giao mong muốn (để trống = giao ngay)"; textSize = 15f; setPadding(dp(12), dp(10), dp(12), dp(10)); background = bg(Color.WHITE, 12) }
        ...
        val previewBtn = button("Xem lại tổng tiền") {
            summaryBox.removeAllViews(); val loadingView = loading(summaryBox, "Đang kiểm tra đơn hàng...")
            executor.execute {
                ...
                runOnUiThread {
                    ...
                    val total = r.optJSONObject("data")?.optInt("total") ?: 0
